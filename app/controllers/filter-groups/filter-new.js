...
export default class FilterGroupsFilterNewController extends Controller {
  ...
  createFilter() {
    ...
    if (this.model.filterGroup.kind === 'numeric') {
      attributes.numericValue = getFormField(
        this.form, 'numeric-value').value;
    }

    this.nonEmberDataApi.createFilter(this.model.filterGroup.id, attributes)
    .then(data => {
      if ('errors' in data) {
      ...
      this.target.transitionTo(
        'filter-groups.show', this.model.filterGroup.get('id'));
    })
    .catch(error => {
