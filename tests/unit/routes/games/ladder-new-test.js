...
import { assertPowerSelectOptionsEqual }
...
module('Unit | Route | games/ladder-new', function(hooks) {
  ...
  hooks.beforeEach( function() {
    ...
    createModelInstance(
      this.server, 'chart-group', {game: this.game, name: 'Chart Group 1'});
    this.chartGroup2 = createModelInstance(
      this.server, 'chart-group', {game: this.game, name: 'Chart Group 2'});
    createModelInstance(
      this.server, 'chart-group', {game: otherGame, name: 'Other Chart Group'});
  });
  ...
  test('can create new ladder', async function(assert){
    ...
    await selectChoose(
      'div.kind-select > .ember-power-select-trigger', 'side');
    fillIn('.filter-spec-input', '2-3n');
    await selectChoose(
      `div.chart-group-select > .ember-power-select-trigger`, 'Chart Group 2');
    // Submit form.
    await click('button[type=submit]');

    // Should redirect to the game's manage-ladders page
    assert.equal(
      currentURL(), `/games/${this.game.id}/ladders`,
    ...
    assert.equal(
      ladder.get('name'), 'New Ladder', 'Should save the expected name');
    assert.equal(
      ladder.get('kind'), 'side', 'Should save the expected kind');
    assert.equal(
      ladder.get('filterSpec'), '2-3n', 'Should save the expected filterSpec');
    assert.equal(
      ladder.get('chartGroup').get('id'), this.chartGroup2.id,
