...
export async function assertPowerSelectOptionsEqual(
  ...
  let optionsList = dropdownContent.querySelector(
    'ul.ember-power-select-options'
  );
  let optionTexts = Array.from(optionsList.querySelectorAll('li')).map(
...
export function assertPowerSelectCurrentTextEqual(
  ...
  assert.equal(
    powerSelectElement.querySelector(`span:first-child`).textContent.trim(),
    expectedText,
    assertMessage
  );
}
