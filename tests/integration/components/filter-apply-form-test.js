...
module('Integration | Component | filter-apply-form', function (hooks) {
  ...
  hooks.beforeEach(function () {
    ...
    this.machineGroup = createModelInstance(this.server, 'filterGroup', {
      name: 'Machine',
      kind: 'select',
    });
    this.blueFalconFilter = createModelInstance(this.server, 'filter', {
      name: 'Blue Falcon',
      filterGroup: this.machineGroup,
    });
    this.whiteCatFilter = createModelInstance(this.server, 'filter', {
      name: 'White Cat',
      filterGroup: this.machineGroup,
    });

    this.settingGroup = createModelInstance(this.server, 'filterGroup', {
      name: 'Setting',
      kind: 'numeric',
    });
    this.setting30Filter = createModelInstance(this.server, 'filter', {
      name: '30%',
      numericValue: 30,
      filterGroup: this.settingGroup,
    });
    this.setting80Filter = createModelInstance(this.server, 'filter', {
      name: '80%',
      numericValue: 80,
      filterGroup: this.settingGroup,
    });

    this.machineGroupAsProperty = modelAsProperty(
      this.store,
      'filterGroup',
      this.machineGroup
    );
    this.settingGroupAsProperty = modelAsProperty(
      this.store,
      'filterGroup',
      this.settingGroup
    );
    ...
    let filters = [
      this.blueFalconFilter,
      this.whiteCatFilter,
      this.setting30Filter,
      this.setting80Filter,
    ];
    this.filterAsMap = (f) =>
      new Map([
        ['id', f.id],
    ...
    this.set(
      'appliedFilterObjs',
      filters.map((filter) => this.filterAsMap(filter))
    );
    this.set('updateAppliedFiltersString', (s) =>
      this.set('appliedFiltersString', s)
    );
  });
  ...
  test('clicking the remove button next to an applied-filter display removes that filter, calling updateAppliedFiltersString() with the appropriate new string', async function (assert) {
    ...
    let expectedAFString =
      `${this.whiteCatFilter.id}` + `-${this.setting30Filter.id}le`;
    assert.equal(
      this.appliedFiltersString,
      expectedAFString,
