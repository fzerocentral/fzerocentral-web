...
module('Integration | Component | record-filters-edit', function (hooks) {
  ...
  test('initializes filters to not-selected', async function (assert) {
    ...
    assert.equal(
      this.element
        .querySelector(
          `div.filter-group-${this.groupA.id}-select .ember-power-select-trigger > span:first-child`
        )
        .textContent.trim(),
      'Not selected'
    );
    assert.equal(
      this.element
        .querySelector(
          `div.filter-group-${this.groupB.id}-select .ember-power-select-trigger > span:first-child`
        )
        .textContent.trim(),
      'Not selected'
    );
  ...
  test('can choose a filter for one group while leaving the other blank', async function (assert) {
    ...
    // Pick one filter
    await selectChoose(
      `div.filter-group-${this.groupA.id}-select .ember-power-select-trigger`,
      'Filter A1'
    );
  ...
  test('can choose filters in both groups', async function (assert) {
    ...
    await selectChoose(
      `div.filter-group-${this.groupB.id}-select .ember-power-select-trigger`,
      'Filter B2'
    );
